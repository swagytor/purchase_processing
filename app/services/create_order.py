...
class CreateOrderService:
    ...
    async def __call__(
        self, user_id: int, item_id: int, max_price: int, quantity: int
    ):
        async with self.db.begin():
            # Получаем стоки товара
            stocks = await self.get_stocks(
                item_id=item_id, max_price=max_price
            )

            if not stocks:
                raise NoResultFound("Нет доступных предложений по товару")

            # Резервируем товар в стоках, пока не выполним нужное количество
    ...
    async def _create_order(
        self, user_id: int, total_reserved: int, order_items: List[OrderItem]
    ) -> Order:
    ...
    async def get_stocks(
        self, item_id: int, max_price: int
    ) -> Sequence[ItemStock]:
