...
async def write_off_order_items():
    pass
...
class ConfirmOrderService:
    ...
    async def __call__(self, user_id: int, order_id: int) -> Order:
        async with self.db.begin():
            ...
            await self.write_off_order_items(order=order)

            order = await self._confirm(order=order)
            await self.db.flush()
    ...
    async def _confirm(self, order: Order) -> Order:
